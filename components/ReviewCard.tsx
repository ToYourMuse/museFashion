...
const ReviewCard = ({ review }: ReviewCardProps) => {
  return (
    ...
    >
      {/* Stars */}
      <div className="flex items-center rounded-full gap-1 mb-4">
        {[...Array(5)].map((_, i) => 
          i < review.stars ? (
            <MdOutlineStar
              key={i}
              className={`text-2xl fill-yellow-400 text-yellow-400"`}
            />
          ) : (
      ...
      {/* Review Text */}
      <p className="text-[16px] font-extralight leading-relaxed mb-4">
        {review.review}
      </p>
